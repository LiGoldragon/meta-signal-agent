...
const MODULE_PATH: &[&str] = &["meta_signal_agent", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let source =
            fs::read_to_string(&source_path).expect("read Agent meta-policy Interface source");
        ...
        .expect("assemble authority-approved Agent meta-policy Interface transaction");
        let rust = rust_logos();
        let type_paths = ProducerRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project Agent meta-policy Interface from the verified transaction")
            .write_or_check("META_SIGNAL_AGENT_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in Agent meta-policy Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("meta-signal-agent")
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("AgentMetaPolicy bootstrap catalog is complete")
}
